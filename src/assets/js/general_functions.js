...
(function (exports) {
    ...
    exports.routeToTab = function() {

        if ( $('a[data-toggle="tab"]').length == 0 ) {
            return
        }

        var route = window.location.href.split('backend')[1]
        var _routes = route.split('/')
        var router = []
        ...
        _routes.forEach(function(item) {
            ...
            if ( item.indexOf('#') != -1 ) {
                var subroutes = item.split('#')

                subroutes.forEach(function(_item) {
                    if (!_item) return
                    router.push(_item)
                })
            } else {
    ...
    exports.updateURLWithActiveTab = function() {

        if ( $('ul[role="tablist"] a[data-toggle="tab"]').length == 0 ) {
            return
        }

        $('ul[role="tablist"] a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
            var tab = $(e.target).attr('href')
            history.pushState({id: tab}, '', tab)
        })
    }
